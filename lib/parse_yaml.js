...
function processBuildpack(build_parameters, name, tag) {
    ...
    if (builders.has(builder)) {
        if(builder.includes('unified-runtime')) {
            let builderType = builder.split('/')[1];
            if(builder.includes('proxy')) {
                builder = `proxy-unified-runtime-dmz.int.repositories.cloud.sap/builder/${builderType}`
            } else {
                builder = `unified-runtime.common.repositories.cloud.sap/builder/${builderType}`;
            }
        } else {
